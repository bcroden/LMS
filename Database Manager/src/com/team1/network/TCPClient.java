...
{
	/**
	 * Allows creation of a TCPClient by only specifying the host name and port number. This will result
	 * in the client blocking while reading data from the server. For a nonblocking client, use the other
	 * constructor
	 * 
	 * @author 						Alex Anderson
	 ...
	 */
	public TCPClient(String host, int port) throws InvalidKeyException, IOException
	{
		this(host, port, 0);
	}
	/**
	 * Allows creation of a TCPClient. This will result a client which will block for a present number of milliseconds
	 * when waiting for the server to send data
	 * 
	 * @author 						Alex Anderson
	 * @param host					String representing the name of the host
	 * @param port					Integer indicating the port number to which the server is listening
	...
	/**
	 * Encrypts a given string and sends it to the server. This method should not be called more than once unless
	 * it failed the previous try.
	 * 
	 * @author 						Alex Anderson
	...
	/**
	 * Attempts to read data sent by the server. If the client is completely blocking, this method
	 * will block until data is sent by the server. If the client has a timeout specified and no data
	 * has arrived by the specified waiting period, this method will return null.
	 * 
	...
	/**
	 * Combines the functionalities of sendRequest(String) and getReply(). This method will send a request
	 * to the server, and block until it receives a reply. This method will always block even if a maximum
	 * wait time is specified in the constructor.
	 * 
	 * @author 						Alex Anderson
	 * @param request				A String representing the request for the server.
	...
	/**
	 * Closes the socket and all input/output streams used to communicate with the server. This method
